...
=head1 FUNCTIONS
   ...
   $si = Rmpfr_fpif_import_mem ($op, $buf, $bufsize); # Needs mpfr-4.3.0
    ...
    sufficiently large to accommodate the output of the function. You can
    calculate the minimum $bufsize with:

      $size = Rmpfr_fpif_size($op);
      $bufsize = Rmpfr_fpif_size($op) + 1; # Include the terminating
                                           # NULL byte
      Or, simply call the functions as:
        Rmpfr_fpif_export_mem($buf, Rmpfr_fpif_size($op) + 1, $op);
        Rmpfr_fpif_import_mem($op, $buf, Rmpfr_fpif_size($op) + 1);
