...
=head1 FUNCTIONS
   ...
   $si = Rmpfr_fpif_import_mem ($op, $buf, $bufsize); # Needs mpfr-4.3.0
    ...
    $stream/$buf in a floating-point interchange format. The import
    function fails if the precision (which is read from the
    stream/string) is greater than MPFR_PREC_MAX.
