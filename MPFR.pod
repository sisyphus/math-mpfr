...
=head1 FUNCTIONS
   ...
   $inex = Rmpfr_rsqrt   ($rop, $op, $rnd);
    ...
    Rmpfr_rsqrt only differs for -0, where it returns -Inf instead of
    +Inf, following the IEEE 754 standard (Section 9.2.1).
   ...
   $inex = Rmpfr_rootn_si($rop, $op, $si $rnd); # mpfr-4.2.0 and later
    Set $rop to the Nth root rounded in the direction $rnd - where N
    is the value held by $ui or (resp.) $si.  If N = 3 then this is
    the cubic root - the same as Rmpfr_cbrt.
    For N odd (resp. even) and $op negative (including
    ...
    These 2 functions (and Rmpfr_cbrt) agree with the rootn function
    of the IEEE 754-2008 standard (Section 9.2).
   ...
   $inex = Rmpfr_tanu($rop, $op, $ui, $rnd); # mpfr-4.2.0 and later only
    ...
    (tanPi).
    Rmpfr_cosu/sinu/tanu   ($rop, $op, $ui,    $rnd) is mathematically
    equivalent to:
    Rmpfr_cospi/sinpi/tanpi($rop, ($op/$ui)*2, $rnd).
   ...
   $inex = Rmpfr_tanpi($rop, $op, $rnd); # mpfr-4.2.0 and later only
    ...
    Rmpfr_cospi/sinpi/tanpi($rop, $op, $rnd) is mathematically
    equivalent to:
    Rmpfr_cosu/sinu/tanu   ($rop, $op, 2, $rnd) when performed with
   ...
   $inex = Rmpfr_legendre($rop, $si, $op, $rnd); # mpfr-4.3.0 &
                                                 # later only
    $rop is set to the value of Legendre's polynomial P_N($op),
    rounded in the direction of $rnd, where N stands for the degree of
    the polynomial - which is set to the value held by $si. If $si is
    negative, then the function aborts with an appropriate message.
    If the input variable $op is outside the [-1,1] domain, then $rop
    is set to 'NaN' and 0 is returned.
   ...
   For Math::MPFR objects these will usually be displayed as 'Inf', -Inf',
   and 'NaN'. When using Rmpfr_(s|f)printf, they will be displayed as
   dictated by the underlying C library - which will likely be 'inf', '-inf'
   and 'nan'.
   If you set $Math::MPFR::PERL_INFNAN to a TRUE value, then nvtoa($num) and
   and numtoa($num) (which are documented below in 'MISCELLANEOUS') will
   display them in the same way that perl  itself displays them.
   ...
   $ui = RMPFR_VERSION_NUM($major, $minor, $patchlevel);
   ...
   $si = Rmpfr_nrandom ($rop, $state, $rnd);  # mpfr-4.0.0 and
                                              # later only
   $si = Rmpfr_nrandom_v1($rop, $state, $rnd); # mpfr-4.3.0 and
                                               # later only
   $si = Rmpfr_nrandom_v2($rop, $state, $rnd); # mpfr-4.3.0 and
                                               # later only
    ...
    These functions are more efficient than Rmpfr_grandom.
    Rmpfr_nrandom and Rmpfr_nrandom_v1 are the same function.
    At some point in the future Rmpfr_nrandom will change to
    invoking the more efficient Rmpfr_nrandom_v2.
   ...
   OPERATOR OVERLOADING
    ...
    For comparison operations between Math::MPFR and Math::GMP objects
    it is best that Math::GMP is at versiomn 2.11 or later && that the
    Math::GMP_OLOAD module has been loaded.
    Otherwise the comparisons will need to be constructed such that
    the Math::MPFR overloading is used - eg you will have to compare
    ($m_mpfr_obj < $m_gmp_obj), not ($m_gmp_obj > $m_mpfr_obj). If
    Math::GMP_OLOAD has been loaded, then the comparisons will work
    either way.
    ...
    (or later), then, for the following operations, $G can be
    either a Math::GMPz or Math::GMPq object - and, if the
    module Math::GMP_OLOAD has been loaded, then $G can also be a
    Math::GMP object if Math::GMP (which is loaded by
    Math::GMP_OLOAD) is at version 2.11 or later.
    Be aware that all of these operations return a Math::MPFR object:
