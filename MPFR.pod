...
=head1 FUNCTIONS
   ...
   $si = Rmpfr_fpif_import_mem ($op, $buf, $bufsize); # Needs mpfr-4.3.0
    ...
    length to accommodate the string written by the function,
    and $bufsize should be set to length($buf). Call fpif_size($op)
    to obtain a (close to minimum) satisfactory value for the size
    of $buf, before making the Rmpfr_fpif_export_mem() call.

    NOTE: If reusing $buf for multiple Rmpfr_fpif_export_mem calls
          make sure that $bufsize is passed as the initial size
          of $buf - and not length($buf), as the latter may
          evaluate to less than the available number of bytes.

   $ui = fpif_size($op); # Needs mpfr-4.3.0
    A fudged calculation of the size of the string buffer that needs
    to be passed to Rmpfr_fpif_export_mem() above. The returned value
    is always large enough but not always the minimum.
    TODO: Always return the minimum value.
