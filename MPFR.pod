...
=head1 FUNCTIONS
   ...
   $str = doubletoa($nv); # Available only when $Config{nvsize} == 8
                          # Else use nvtoa().
    ...
    but reliable.
    An alternative fallback (achievable by providing the DTOA=S
    argument to the 'perl Makefile.PL' step of the Math::MPFR build)
    will ensure that sprintf("%.16e", $nv) is returned. This will
    be an accurate return, and it will be quicker than the nvtoa()
    fallback, but it often provides more digits than are necessary.
    Whenever doubletoa falls back to the nominated fallback routine,
    $Math::MPFR::doubletoa_fallback will be set to 1. This feature
    can be disabled by providing the FB=0 argument to the
   'perl Makefile.PL' step of the Math::MPFR build.
