...
=head1 FUNCTIONS
   ...
   $si = Rmpfr_round_nearest_away(\&function, $rop, @input_args);
    This function might not correctly handle values in the
    vicinity of 0.5 * (2 ** Rmpfr_get_emin()). If you are working
    with values of that order you should first run something like:
