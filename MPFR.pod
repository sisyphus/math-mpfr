...
=head1 FUNCTIONS
   ...
   @data = get_exact_decimal($op);
    Return a 3-element array that allows construction of an exact base
    10 representation of the base 2 value held in $op.

    $data[0] contains the base 10 mantissa (with an implicit decimal
    point before the leading decimal digit.

    $data[1] contains the exponent of this exact decimal value.

    $data[2] contains the number of digits contained in $data[0], or -1
    if $op is a zero, inf, or nan.

    The user can then pass this array as arguments to the decimalize
    function (below) which will convert this information into a usual
    string representation of this exact decimal value.
    Alternatively, if the user is unhappy with the formatting provided
    by decimalize, then the 3 arguments can be passed to a customized
    routine instead.

   $str = decimalize(get_exact_decimal($op));
    Return an exact base 10 representation of the value held in $op.
    Alternatively, if one has already run:
     @data = get_exact_decimal($op);
      then one can avoid re-running get_exact_decimal() by running:
     $str = decimalize($data[0], $data[1], $data[2]);

   $bool = check_exact_decimal($op, get_exact_decimal($op));
    Check that the value obtained by decimalize() is correct. This is
    achieved by assigning the exact decimal string value to a
    temporary mpfr_t object and checking that:
      a) the assignment was exact;
      b) $op == the temporary object.
    Again, if get_exact_decimal($op) has already been run:
     @data = get_exact_decimal($op);
      then one can avoid re-running get_exact_decimal() by running:
     $str = check_exact_decimal($op, $data[0], $data[1], $data[2]);
