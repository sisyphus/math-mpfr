...
=head1 FUNCTIONS
    ...
    *minimum* number of digits such that the condition ($str == $nv)
    is true.
    NOTE 1: For perls whose NV is the double-double, $str may contain
            more digits than is necessary. (Work in progress.)
    NOTE 2: Perl versions prior to 5.29.4, or subsequent versions that
            ...
            looks only at the value that was passed to it - which, with
            such buggy perls, may differ from the intended value.
