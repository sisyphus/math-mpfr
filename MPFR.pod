...
=head1 FUNCTIONS
   ...
   $rop = subnormalize_bfloat16($val);
    ...
    For "float32" (ie the single precision "float"):
     ...
     value held by $val, adjusted for "float" subnormalization
     when appropriate.
    For "float16":
     Returns an 11-bit precision Math::MPFR object containing the
     value held by $val, adjusted for "_Float16" subnormalization
     when appropriate.
    For "bfloat16:
     Returns an 8-bit precision Math::MPFR object containing the
     value held by $val, adjusted for "__bf16" subnormalization
     when appropriate.
