...
=head1 FUNCTIONS
   ...
   $ret = decimalize($op [, $anything]); # 2nd (optional) arg
    ...
    representation of the value held in $op.
    Else return the no. of decimal digits (not counting leading zeros)
    contained in the significand of that "exact base 10 representation",
    as calculated by decimalize(). (This value can be one greater than
    the actual number of those significand digits.)
